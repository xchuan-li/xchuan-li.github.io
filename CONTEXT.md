...
**Positioning (repositioned 2026-07-13, current).** The identity is the **experimental and computational semantics of modal reasoning**: *how linguistic modality structures the space of possibilities for inference, deriving predictions from formal semantics and testing them in both the human mind and language models*. Modality is now the **whole focus** for the MSc stage — not the first of three case studies. The site is built around the **Master's thesis in two co-equal arms**: (1) a **language-model arm** (MODUS — is modal force a set of possible worlds à la Kratzer, or a degree of certainty à la Lassiter?; a pilot runs, the model is the instrument, read under intervention); (2) a **human arm** — the same modal contrast put to people through a shared behavioural paradigm, drawing on the developmental modal-cognition literature (Leahy & Carey), presented as a **live planned arm**, institution-level only, **no collaborator names**. The theoretical spine both arms derive from is a **formal-semantics working paper on modality (in preparation)**. Lineage: **Leibniz → Kripke → Kratzer → Leahy & Carey**. Interpretability (do-interventions, probing, activation patching) is the **measurement layer / instrument**, not the identity. The two earlier collaborations — **Cross-lingual ProtoBias** (multilingual VLM evaluation) and **Latent Control States / DFKI** (mechanistic interp) — are kept, demoted to an **"earlier & methodological work"** tier (the engineering range the program runs on), never the headline.

**Removed from the site (2026-07-13), pages orphaned not deleted:** the morphology case study **HanGL** (`/research/hangl`) and the generics case study **ORDO** (`/research/ordo`), plus the old three-case-study hero `HeroCaseStudies.astro` (replaced by `HeroModality.astro`). Their page files and figure/demo components remain in the repo but are unlinked from every listing, nav, and data source — restore by re-adding to `data/research.ts` and the page maps.

**Red lines — do NOT drift back.** (1) Do NOT reintroduce the three-case-study framing ("one question, three kinds of word"; modals/compounds/generics) — the MSc identity is modality only (`硕士阶段我的研究就围绕模态`). (2) Never frame the site *as* "causal inference" / "grounding certification" / "CIY" — those are methods the program uses. (3) The narrower tagline *"how linguistic modality structures possibilities for inference … in humans and language models"* is fine (modality + formal-semantic predictions stay central); the too-broad *"how linguistic structure scaffolds reasoning in humans and AI"* / "language and reasoning" is still forbidden (no falsifiable criterion). (4) The human arm is presented as real/planned but **institution-level, no names**; do not name collaborators publicly.
