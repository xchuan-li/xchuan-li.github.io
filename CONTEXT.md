...
**Homepage structure (17 September 2026).** Header → Research program → Selected research → Background timeline → Writing → Playground → Contact. Between the research program and the timeline, use one restrained list with three entries: the related formal analysis and proposed human thesis, Cross-lingual ProtoBias as completed model evaluation, and Latent Control States as ongoing mechanistic interpretability training. Use short descriptions, honest status lines, and links to detail pages. Avoid numbered stage cards, duplicate project-group introductions, and a separate exploratory-work section. The author subsequently requested the original ProtoBias cover and the Connection/Motive margin annotations back: retain these within the compact structure. On wide screens, notes sit in the outer margins; on intermediate screens, in the existing left column; on phones, below the associated entry.
